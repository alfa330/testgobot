...
async def send_welcome(message: types.Message):
    welcome_text = (
        ...
        "Жұмыс және еркін қозғалыс үшін электр велосипедтерін жалға алу!\n\n"
...
async def show_options(message: types.Message, user_id: int):
    ...
    keyboard.add(InlineKeyboardButton(
        "Помощь с велосипедом 🛠️" if lang == "lang_ru" else "Велосипедке көмек 🛠️",
...
async def show_rental_options(message: types.Message, user_id: int):
    ...
    rental_text = (
        ...
        if lang == "lang_ru" else
        "<b>Электр велосипедті жалға алу 🚴‍♂️</b>\nЖалға алу, жинақтама немесе сатып алу туралы мәліметтерді біліңіз!"
    )
    ...
    keyboard.add(InlineKeyboardButton(
        "Выкуп велосипеда 💸" if lang == "lang_ru" else "Велосипедті сатып алу 💸",
        callback_data="rent_purchase"
    ))
    keyboard.add(InlineKeyboardButton(
        "Консультация у менеджера 📞" if lang == "lang_ru" else "Менеджермен кеңесу 📞",
        url=whatsapp_link
    ))
    keyboard.add(InlineKeyboardButton(
...
async def process_selection(callback_query: types.CallbackQuery):
    ...
    if callback_query.data == "back_to_options":
        ...
        details_text = (
            ...
            if lang == "lang_ru" else
            "<b>Электр велосипедті жалға алу шарттары 🚴‍♂️</b>\n\n"
            "Жеткізу қызметінде жұмыс істейсіз бе? Табысыңызды арттырып, шығындарды азайтқыңыз келе ме?\n"
            "<b>Электр велосипедті аптасына бар болғаны 20 900 ₸-ға жалға алыңыз</b> және кептелістерді, транспортты және артық шығындарды ұмытыңыз!\n\n"
            "🔥 Кепілсіз\n"
            "🔥 Жылдам беріледі\n"
            "🔥 Сенімді техника\n"
            "🔥 24/7 қолдау\n"
            "<b>Үшін өте қолайлы:</b> Яндекс.Еда, Glovo, Wolt және т.б.\n"
            "<b>Берілетін мекенжай:</b> Сәтбаев 35А (9:00-19:00)"
        )
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton(
            "Консультация у менеджера 📞" if lang == "lang_ru" else "Менеджермен кеңесу 📞",
        ...
        purchase_text = (
            ...
            if lang == "lang_ru" else
            "<b>Алматыда электр велосипедті сатып алу 🚴‍♂️</b>\n\n"
            "Өз электр велосипедіңді ал — жұмыс істе және сатып ал!\n"
            "<b>📍 Қайдан алуға болады?</b> Сәтбаев 35А, Eazy Go кеңсесі\n"
            "<b>💰 Шарттар:</b>\n"
            "• Аптасына бар болғаны 37 500 ₸\n"
            "• 5 айда сатып алу — және велосипед сенікі!\n"
            "• Күшті, 2 аккумулятормен және толық жинақтамамен 🔋🔋\n"
            "⚡️ Курьер және Жеткізу тарифтерінде шектеусіз жұмыс істе — велосипед өзін-өзі өтей отырып, көбірек табыс тап!\n"
            "<b>📦 Саны шектеулі — бірінші болып алып үлгер!</b>\n"
            "<b>📞 Жалға алу үшін байланыс:</b> +7 700 808 80 60"
        )
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton(
            "Консультация у менеджера 📞" if lang == "lang_ru" else "Менеджермен кеңесу 📞",
        ...
        help_text = (
            ...
            if lang == "lang_ru" else
            "<b>Велосипедке көмек керек пе? 🚴‍♂️</b>\n"
            "Кез келген сұрақты тез шешу үшін WhatsApp арқылы қолдау қызметіне хабарласыңыз!"
        )
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton(
